---
title: "Esophagus REDCap Cohort"
output: 
  rmarkdown::html_vignette:
    df_print: kable
    toc: TRUE
vignette: >
  %\VignetteIndexEntry{Esophagus REDCap Cohort}
...
```{r secretsetup, echo=FALSE,eval=TRUE,message=FALSE,results="hide", warning=FALSE}
library(fantasia)
...
**Esophagus Cohort Table** 
`patelm9.esophagus_cohort`:  
...
esophagus_cohort[1:20,]

```

**OncoRegimenFinder Regimen Ingredient Table**  
`patelm9.oncoregimenfinder_regimen_ingredient`:  
...
esophagus_oncoregimenfinder_results[1:20,]
```
...
Analysis of OncoRegimenFinder results shows that `r unlist(data_summary[data_summary$Variable == "person_id", "DISTINCT_COUNT"])` Person Ids out of the `r unlist(data_summary[data_summary$Variable == "esophagus_person_id", "DISTINCT_COUNT"])` unique Person Ids in the Esophagus Cohort were resulted from OncoRegimenFinder.  
...
## Notably Incorrect 
```{r}
final_output$`1558617`
```

```{r}
final_output$`1557156`
```
