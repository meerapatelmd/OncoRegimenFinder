---
title: "Esophagus REDCap Treatments vs OncoRegimenFinder Results"
...
## Summary 
The results of the OncoRegimenFinder algorithm is compared to the abstracted Esophagus REDCap cohort treatment data. 

## Esophagus Cohort OncoRegimenFinder  
### Source Tables:  
    1. Esophagus Cohort Table  
    1. OncoRegimenFinder Regimen Ingredient Table
    
An Esophagus Cohort table is currently present in the OMOP `patelm9.esophagus_cohort` table: 
...
The `patelm9.oncoregimenfinder_regimen_ingredient` table is as follows: 
```{r}
...
Analysis of OncoRegimenFinder results indicates that approximately `r data_summary[data_summary$Variable == "esophagus_person_id", "DISTINCT_COUNT"]` patients based on the number of Person Ids in the Esophagus Cohort table while the unique Person Ids found in the results total `r data_summary[data_summary$Variable == "person_id", "DISTINCT_COUNT"]`. 
...
## REDCap Data  
```{r redcap_data,echo=FALSE,eval=TRUE,message=FALSE, results='hide'}
...
```{r redcap filter, echo=FALSE,eval=TRUE}
...
`r output %>% purrr::keep(~ncol(.)>4) %>% length()` Variables have more than 1 set of start-stop dates for treatments due to contextual differences.
