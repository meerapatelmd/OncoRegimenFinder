...
UNION
SELECT person_id, drug_era_id, concept_name, ingredient_start_date FROM @writeDatabaseSchema.@regimenTable WHERE drug_era_id NOT IN (SELECT drug_era_id FROM regimens_to_keep)
)
